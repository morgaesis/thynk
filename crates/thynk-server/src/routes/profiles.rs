...
fn build_mutual_work(db: &thynk_core::Database, target_username: &str, current_username: &str) -> Vec<NoteActivity> {
    let all_notes = db.list_notes().unwrap_or_default();
...
mod tests {
    ...
    async fn test_get_user_profile_by_username() {
        ...
        let state = {
            ...
            db.create_user("user1", "alice", "alice@example.com", Some("Alice"), "2024-01-01T00:00:00Z").unwrap();
            db.create_user("user2", "bob", "bob@example.com", Some("Bob"), "2024-01-01T00:00:00Z").unwrap();
    ...
    async fn test_get_user_profile_mutual_work() {
        ...
        let state = {
            ...
            db.set_note_links("alice-note", &["bob-note".to_string()]).unwrap();
